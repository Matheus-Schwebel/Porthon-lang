...
from porthon import ast
from porthon.lexer import Lexer, TokenStream
from porthon.parser import Parser
from porthon.errors import AbrvalgSyntaxError, report_syntax_error
from porthon.utils import print_ast, print_tokens, print_env
import os
import sys
import subprocess
...
def add_code(filename, env, verbose=False):
    if os.path.exists(filename):
        with open(filename, 'r') as arquivo:
            code = arquivo.read()
            evaluate_env(code, env, verbose)
    else:
...
def add_builtins(env):
    builtins = {
        ...
        'imprimir': (['value'], lambda args, e: print(args['value'])),
        'len': (['iter'], lambda args, e: len(args['iter'])),
        'slice': (['iter', 'start', 'stop'], lambda args, e: list(args['iter'][args['start']:args['stop']])),
        'fatia': (['iter', 'start', 'stop'], lambda args, e: list(args['iter'][args['start']:args['stop']])),
        'str': (['in'], lambda args, e: str(args['in'])),
        'string': (['in'], lambda args, e: str(args['in'])),
        'inteiro': (['in'], lambda args, e: int(args['in'])),
        'int': (['in'], lambda args, e: int(args['in'])),
        'importar': (['filename'], lambda args, e: add_code(args['filename'], e)),
        'sair': (['number'], lambda args, e: sys.exit(args['number'])),
        'entrada': (['text'], lambda args, e: input(args['text'])),
        'shell': (['command'], lambda args, e: shell(command=args['command'])),
